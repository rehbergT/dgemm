#' Description of the dgemm_C_loops_avx_tp function
#'
...
#' c <- dgemm_C_loops_avx_tp(a, b)
#' @useDynLib dgemmR, .registration = TRUE
#' @export
dgemm_C_loops_avx_tp <- function(matrix_a, matrix_b, repeats = 1) {
    to_C <- list()
    to_C$matrix_a <- matrix_a
    to_C$matrix_b <- matrix_b
    to_C$repeats <- as.integer(repeats)

    result <- .Call("_dgemm_C_loops_avx_tp", to_C, PACKAGE = "dgemmR")
