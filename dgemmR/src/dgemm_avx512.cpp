...
void dgemm::dgemm_C_loops_avx512(double* matrix_a,
                                 double* matrix_b,
                                 double* result,
                                 int M,
                                 int K,
                                 ...
                                 int parallelization) {
    if (verbose)
        PRINT("Using AVX512 version\n");

    if (!__builtin_cpu_supports("avx512f")) {
        if (verbose)
            PRINT(
                "CPUs does not support AVX512. Defaulting to fallback "
                "version.\n");
        dgemm_C_fallback(matrix_a, matrix_b, result, M, K, N, repeats, verbose);
        return;
    }

    int alignedDoubles = 8;
    int alignment = 64;
